...
> 📦 本節與下節「主迴圈交付資料的義務」於 2026-09-02 自 Meridian repo `WORKFLOW.md` 第 22–59 行**整段搬入**(架構師裁決:全專案適用;抄本經 md5 對照原檔)。文中 M1-x 為 Meridian 里程碑事故,其他專案讀作判例。

> ⚠️ 原條文是「知識庫只有協作 Claude 讀寫,**Claude Code 連讀都不碰**」。
> 三方編排把協作側搬進 Claude Code 行程之後,那個講法已經對不上實體;
> **架構師 2026-08-24 拍板改為以行程分**:

- ✅ **主迴圈(Claude Code 主行程)讀寫皆可** —— 它是 vault 的唯一維護者。
  原先掛在 `nr-planner` 的「收案後更新 vault」職責**已收回主迴圈**。
- ⛔ **所有子代理一律不可**(`nr-planner` / `nr-implementer` / `nr-auditor`,讀寫皆擋),
  由 `vault-guard.sh` 以 `agent_type` 白名單強制。
- ⚠️ **為什麼子代理要擋到底**:實作側隔離的價值在於「任務包沒寫的規格就是**缺失**」——
  包漏寫時會以「子代理卡住回報」的形式**當場現形**。子代理一旦能自己去 vault 補洞,
  那個訊號就消失了,而它是這條驗證鏈上唯一會主動報錯的地方。
- ⛔ **主迴圈可讀不等於可轉手**:vault 內容⛔ 不得抄進交給子代理的 prompt,
  也⛔ 不得寫進任務包(那等於繞過閘門讓子代理讀到,並侵蝕「任務包自我完備」)。
  ⚠️ 這一條**機器驗不了**,是紀律;閘門只保證「誰**直接**存取得到」。
- ⚠️ 本閘門唯一的**不安全**失效方向是「`agent_type` 欄消失 ⇒ 全被當成主迴圈放行」,
  已由 `gate-selfcheck` 的「子代理樣本必須被擋」斷言釘住(實測會轉紅)。

---

### 主迴圈交付資料的義務(2026-08-25 增設,M1-7 實測促成)

> ⚠️ 五支閘門管的全是「子代理的**權限**」,**沒有任何一支管主迴圈產出的資料**。
> 而主迴圈是唯一能讀 vault、又直接餵資料給子代理的角色 —— 這是編排裡唯一無防護的位置。

- **凡主迴圈提供數值,必須附產生它的確切指令。** 那條指令的角色是**指路**,⛔ 不是結論。
- **署名即重量**:任何數字要進入誰的產出,就由**誰自己量一次**;⛔ 不論來源
  (主迴圈、任務包、plan、上一輪報告)一律不得直接轉抄。
  已寫進三個子代理的鐵則(`nr-implementer` 7 / `nr-auditor` 6 / `nr-planner` 8)。
- **界線**:**指令性**內容(做什麼、範圍、裁決甲乙丙、檔案路徑、停手條件)以主迴圈為權威,照做;
  **量測性**內容(位元組數、行數、計數、速度、雜湊、bytecode 偏移)才適用本條。
- ⚠️ **為什麼要立**:M1-7 一輪內同類問題出現**三次** —— 埠 `25577` 自 plan 轉抄、
  主迴圈把**兩個未量測**的速度值填進表格交給實作方寫報告、驗收報告三條彙總計數 ——
  **無一自查發現**,全部由下一環抓到。鏈條有效,但各環自查不足。
- ⚠️ 這一條**機器驗不了**(沒有 hook 檢查得了「這個數字量過沒有」),
  它的執法者是**下一環的重量義務**,⛔ 不是主迴圈的自律。

---

## 檔案管道(五件套)
> 📦 本章於 2026-09-02 自 Meridian repo `WORKFLOW.md` 第 62–134 行**整章搬入**,取代原「四件套」章(架構師裁決:全專案適用;抄本經 md5 對照原檔)。文中 M1-x 為 Meridian 里程碑事故,其他專案讀作判例。
...
| `{代號}-verification-audit.md` | 協作 Claude(審計:抽查結果、必改明標 ⛔、追認、報告更正) | Claude Code |

> 第五件於 M1-3 增設(架構師 2026-08-18 裁決甲):審計原只存在於對話,
> 實作方在檔案管道上拿不到必改項——違反「所有 context 活在檔案裡」的設計。
> 審計結論只有兩種:**通過**(可進 commit 裁決)/**有條件通過或不通過**(必改逐項列、修畢回報)。

**歸檔慣例**:任務收案後,該任務五件套全數移入 `docs/tasks/archive/`(按任務歸檔)。
`docs/tasks/` 根層只留骨架模板與**進行中**任務——目錄列表本身就是「目前在做什麼」的答案。

> ⚠️ **歸檔搬移由架構師手動執行(2026-08-25 定案,M1-7 實測促成)**:編排裡**沒有任何角色**
> 搬得動那兩個批覆檔 —— `review-write-guard` 的寬擋讓 `git mv` 的 `mv` 命中,而白名單只放
> `nr-planner` / `nr-auditor`;前者**沒有 Bash**、後者被 `audit-write-guard` 擋死一切檔案改動;
> 主迴圈與 `nr-implementer` 落不進白名單。**四個角色逐一實測皆 deny/不可行。**
> ⛔ **不為此放寬閘門** —— 例外正好會開在「檔案管道全部價值所在」的那兩個檔上,不划算;
> 歸檔一輪只發生一次,人工成本可忽略。收案時架構師執行 `git mv` 五件套 + commit。

> ⛔ **寫入權鐵律(2026-08-19 增設,事故促成)**:上表「產出者」欄即寫入權歸屬——
> Claude Code ⛔ 絕不得寫或改 `{代號}-plan-review.md` 與 `{代號}-verification-audit.md`
> 的任何內容,**尤其不得代填架構師批覆、裁示結果、審核結論**——批覆記錄由非裁決方
> 寫入即屬偽造,內容碰巧正確不改變性質。需回報事項一律寫進自己產出的檔
> (plan / verification)或另立回報檔,等對方處理。
> ⚠️ 知識庫(vault)那半句**已於 2026-08-24 改判**,見上節「vault 讀寫權」——
> ⛔ 不再是「Claude Code 連讀都不碰」,而是「主迴圈讀寫皆可、子代理一律不可」。
> (事故記錄:M1-4 Plan 審核期,實作方代填 review 批覆並寫入知識庫 Decision Log——
> 內容經核與實際定案一致、已追認,但行為本身即驗證鏈破口;見 PROGRESS 2026-08-19 登記。)

### 批覆回填規則(2026-08-24 增設,新編排促成)

> ⚠️ **為什麼要補這一條**:上面的鐵律成立於「協作 Claude 與 Claude Code 是兩個行程、
> 兩個對話」的前提。M1-7 產包時,三方編排把協作側(`nr-planner`)搬進 Claude Code 行程內跑,
> 於是「誰寫的」這條界在檔案系統上**不再有任何機制對應**——同一個行程裡,
> 主迴圈與實作側同樣寫得到那兩個檔。鐵律本身沒錯,是它的**前提**被搬動了。
> (架構師 2026-08-24 拍板:改 WORKFLOW + 加閘門,⛔ 不維持現狀。)

1. **誰寫得到**:plan-review 由 `nr-planner`(規劃側)寫;verification-audit 由 `nr-auditor`
   (稽核側)寫。⛔ **主迴圈與 `nr-implementer` 一律寫不到**(由 `review-write-guard.sh` 強制),
   ⛔ 連「內容完全正確、只是順手落檔」也不行。**讀取不受限制**——
   實作側必須讀得到 review 才知道要改什麼。

   ⚠️ **機制細節(實測,⛔ 非推測)**:閘門靠 PreToolUse payload 的 `agent_type` 辨識發動者
   (子代理多出 `agent_type` / `agent_id` 兩欄,主迴圈皆 `null`;`session_id` 與
   `transcript_path` 主子**完全相同** ⇒ ⛔ 分不出誰是誰)。白名單是 `nr-planner` / `nr-auditor`,
   其餘一律擋。⚠️ 同一次取樣也證實 **`settings.json` 的 hook 會下沉到子代理** ——
   ⛔ 沒有「只掛主迴圈」這種掛法(第一版就是這樣把 `nr-planner` 自己擋死的),白名單是唯一解。
2. **批覆怎麼進檔**:架構師在對話中拍板後,由 `nr-planner` 回填「裁決記錄」欄,且**必須**:
   - **逐字轉錄**架構師的原話或所選選項,⛔ 不得改寫、⛔ 不得補充理由當成架構師說的;
   - 標明**來源與日期**(格式:「架構師 YYYY-MM-DD 於對話拍板」);
   - 甲/乙/丙**選項原文保留存查**(「為什麼不選甲/乙」與「為什麼選丙」一樣值錢);
   - 若批覆與協作方建議不同,⛔ 不得暗示是協作方自選。

   ⚠️ 「是不是逐字轉錄」**機器驗不了**,這一條仍是紀律;閘門只保證**誰寫的**。
   ⇒ 架構師抽查批覆欄時,對的比較基準是**對話原文**,⛔ 不是檔案自己說它逐字。
3. **沒拍板就是沒拍板**:裁決點未獲批覆時,裁決記錄欄一律留「⏳ 待批覆」。
   ⛔ 不得因為「建議很明顯會被採納」或「先寫好省時間」而預填。
4. **閘門會誤殺**:`review-write-guard` 是寬擋的——指令字串同時出現目標檔名與
   寫入跡象(重導向 / 複製 / 原地編輯 / …)就擋,即使那只是備份或 grep 樣式。
   ⚠️ 撞到時的正確反應是**把指令拆開**或改用檔案編輯工具,⛔ 不是放寬閘門。
   (增設當天實際撞到兩次,兩次都是備份指令與條文內容同處一行。)
5. **閘門自檢**:第五支閘門已納入 `gate-selfcheck.sh`(SessionStart 每 session 驗一次:
   檔案完整性 / 必中樣本吐 deny / fail-closed / settings 與 frontmatter 接線 /
   ⛔ `nr-planner` **不得**被誤掛)。⚠️ 改過 `.claude/agents/*.md` 必須**重開 session**,
   同 session 內驗閘門會得到假結果。
...
7. **更新 PROGRESS.md**；里程碑收尾走下方 checklist；收案五件套歸檔。
...
- [ ] 該里程碑所有子任務都 ✅（含補驗過的 🔴 已轉 ✅）。
- [ ] 收案任務的五件套移入 `docs/tasks/archive/`（可併入下一輪實作 commit 或獨立 `docs:` commit）。
