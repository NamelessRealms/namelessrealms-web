...
## ⚠️ 附錄：N1b 的第三方獨立重跑（2026-09-09，稽核後追加）

> ⚠️ **本節是稽核完成後才追加的**，⛔ 不是原報告內容。追加者：主迴圈。

**為什麼追加**：本報告的執行者是主迴圈本人，而稽核側依 `audit-write-guard` 設計
⛔ 不能改檔 ⇒ 需要放 canary 的條目（N1／N1b／N2／P3'）它**結構上跑不動**。
⇒ N1b 守的是**發版路徑**，份量最重，架構師 2026-09-09 裁定由**第三方**（`nr-implementer`，
⛔ 非本報告作者）獨立補跑。⚠️ 該次派工明確要求它**先不看本報告怎麼寫**，
自行設計、自行取證，最後才比對。
...
| 項 | 結果 |
|---|---|
| 取值機制負向對照 | 先證明 `out=$(cmd 2>&1); st=$?` 在本機 zsh 下真的取得非 0（`st=1`） |
| 起始 `yarn build` | `exit=0`，13 條路由全數產出 |
| canary | **新建** `components/NrCanaryN1b.tsx`（264 B），觸發 `@next/next/no-sync-scripts`（error 級）⛔ 未改任何既有檔 |
| 加 canary 後 `yarn build` | **`exit=1`**、`Failed to compile.`，訊息精確指向 `6:7 … @next/next/no-sync-scripts` |
| 加 canary 後 `yarn lint` | `exit=1`（不加旗標亦然）⇒ 順帶證實 **error 級⛔ 不需要 `--max-warnings 0` 就會擋**；該旗標守的是 warning 級（＝ N2） |
| 刪除 canary 後 | `yarn build` `exit=0`；動工前／後兩次 `git status --short` **逐字相同** |

⇒ **與本報告 N1b 相符。** 它獨立選定的規則、放置目錄、exit code 與錯誤訊息原文均與本報告一致，
差別僅在 canary 檔名。⇒ 本報告的 N1b 是**可重現的事實**，⛔ 不是設計推理。

⚠️ **界線（據實記）**：第三方**只重跑 N1b（順帶 N1）**，
對 P1a／P1b／P2／N2／N3／P3' 各條**⛔ 未複驗、⛔ 不背書**。
⇒ 那幾條目前仍只有主迴圈跑過（稽核側另以 `-c` 指向既有 preset 的唯讀槓桿，
獨立取得過一次「閘門真的會 fail」的負向對照 `exit=1`，見 `F8-verification-audit.md`）。

⚠️ **一個未追查的觀察（⛔ 不寫成結論）**：第三方兩次 build 的 middleware 大小分別印
`74.9 kB` 與 `74.8 kB`，而 `middleware.ts` 全程未被修改（git 可證）。
推測為 build 輸出的非決定性，但**⛔ 未實查**，據實留記。
