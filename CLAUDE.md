...
8. **工程慣例 checklist（每次實作 + 驗收報告須一併滿足）**：
   ...
   - 📦 **共用 checklist 條**（既有註解全稱比對、盤點指令不用 `&&`、有正控 ≠ 有效、殼層 `grep -r`、執行期落檔）由下方「撰碼規約」節 `@` 引用的組織層共用檔提供，**與本清單同效力**；⛔ 不在本檔重抄。
...
- **§C 抽共用的落點（本專案）**：共用檔只說「必須抽共用」、⛔ 未指定放哪——本 repo 一律放 `lib/` 或 `components/`。
  ⚠️ 往 `lib/` 放第一個檔時必須同時把 `'lib'` 加回 `next.config.js` 的 `eslint.dirs`（F10 已把它拿掉），否則 `lib/` 永遠不會被 lint（§B 同條）。
- **§D 登記（不得抽離的邏輯）**：
  - ⚠️ 目前（2026-08-30）**尚未盤點**，⛔ 不代表沒有。
- **本地門檻**：無——反向檢查的跳檔數與共用檔相同（皆為 2），⛔ 未另訂。
